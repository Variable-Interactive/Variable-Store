...
#	var rect := _get_result_rect(origin, dest)
	...
	var color = Tools.get_assigned_color(button)
	_last_pos = _original_pos
	for point in points:
		if point.x - _last_pos.x < 0:
			color.r8 = clamp(color.r8 - 1, 0, 255)
		elif point.x - _last_pos.x > 0:
			color.r8 = clamp(color.r8 + 1, 0, 255)
		if point.y - _last_pos.y < 0:
			color.g8 = clamp(color.g8 - 1, 0, 255)
		elif point.y - _last_pos.y > 0:
			color.g8 = clamp(color.g8 + 1, 0, 255)
